from fastapi import FastAPI, Query, HTTPException, status, Path, Body, Request, Form
...
def add_cars(
    ...
    body_cars = [Car(make=make,year=year,price=price,engine=engine,sold=sold,autonomous=autonomous, model=model)]
    if len(body_cars) < 1:
...
def update_car( request: Request,
    ...
    car = Car(make=make,year=year,price=price,engine=engine,sold=sold,autonomous=autonomous, model=model)
